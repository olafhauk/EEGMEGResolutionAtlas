...
# timing of computations for resolution matrix and metrics
timing = {'R': [], metric: []}
...
for sbj in sbj_ids:
    ...
    for functype in stcs.keys(): # PSF/CTF
        ...
        for inv_type in C.res_inv_types + [x+'-'+y for (x,y) in C.meth_contr] + ['dep'+str(int(100*x))+'-'+str(int(100*y)) for (x,y) in C.meth_contr_dep]: # inverse methods
            ...
            for lambda2 in C.res_lambda2s: # regularisation parameters
                ...
                for modal in C.res_modalities + [x+'-'+y for (x,y) in C.modal_contr]: # ['EEGMEG', 'MEG', 'EEG', 'EEGMEG-MEG', 'EEGMEG-EEG', 'MEG-EEG']:
            ...
            for depth in C.inv_depth: # depth weighting
                ...
                for inv_type in C.res_inv_types: # 'MNE', 'LCMV' etc.
                    ...
                    for lambda2 in C.res_lambda2s: # regularisation parameters
                        ...
                        timing['R'].append(t2-t1)

                        print('###\nThis resolution matrix took %fs to compute.\n###\n' % timing['R'][-1])
                        ...
                        for (axis, functype) in [(0, 'PSF'), (1,'CTF')]:
                            ...
                            timing[metric].append(t2 - t1)

                            print('### Metric %s took %fs to compute.\n###\n' % (metric, timing[metric][-1]))
    ...
    print('###\nResolution matrices took on average %f seconds.' % np.mean(timing['R']))
    print('Metrics took on average %f seconds.' % np.mean(timing[metric]))
